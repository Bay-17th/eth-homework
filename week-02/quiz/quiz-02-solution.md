...
-->
B)가 맞으며, gasLimit이 아닌 gasPrice가 높을수록 트랜잭션이 빨리처리된다.
총 가스비용은 gasPrice와 gasLimit을 곱한 값으로 계산된다.
---
...
-->
B)
트랜잭션 재사용 공격을 방지하기 위해서는 nonce를 통재 트랜잭션을의 순서를 보장하고 같은 nonce를 한번만 처리해야한다.
이 경우 동일한 계정이므로 nonce가 트랜잭션의 순서를 보장해준다.
따라서 B)의 순서가 알맞다. 
...
-->
B)
인증이란 해당 트랜잭션이 계정 소유자로부터 왔음을 증명해준다.
부인방지는 이미 서명을 한 트랜잭션의 내용을 이후에 부인할 수 없도록 한다.
---
...
-->
C), secp256k1타원곡선을 이용한 계산방식은 개인키에서 공개키를 구하는 것은 쉽다.
하지만 역으로 추적해내는 것은 수학적으로 불가능에 가깝고 범위도 매우 크기때문에 공개키를 이용해 개인키를 알아내는 것은 불가능하다.
...
-->
만약 내가 서명한 트랜잭션을 다른 사람이 복사하여 여러번 보내고, 이것이 모두 실행된다면, 원하지 않는 트랜잭션이 여러번 실행되고, 계좌에서 모든 ETH가 빠져나갈 수도 있다. nonce를 이용해, 트랜잭션의 순서를 정해놓으면서, 동일한 nonce값은 한번만 실행하는 것으로 중복된 트랜잭션을 방지할 수 있다.
...
-->
은행은 중앙화된 거래소가 있기 때문에, 범법행위, 이상거래가 발생시 권한으로 취소하고 문제를 해결할 수 있다.
하지만 블록체인에서는 탈중앙화된 방식으로, 이러한 사고들을 중재할 권한을 가진 이가 없기 때문에, 개인키가 유출되어 피해를 보더라도 복구와 같은 문제해결이 불가능하다.
...
이후: 어떤 부분이 달라졌나요?
-->
EIP-1559 이전에는 사용자가 gasPrice를 직접 제시하였다.
EIP-1559 이후에는 수수료를  BaseFee과 PriorityFee의 합으로 한다.
basefee는 네트워크 혼잡도에 따라 프로토콜이 자동으로 조정을 해주고, 모든 트랜잭션에 동일적용이 된다.
priorityFee는 사용자가 선택하는 인센티브로 결정된다.
...
-->
ETH입금의 msg.sender를 user로 변경해주기 위해 vm.prank(user);를 사용하고, 입금시에는 deposit함수에서 balances에 더할 value를 지정하여 함수를 호출해준다.
...
어떤 검증이 빠져 있나요?
-->
잔액이 충분한지 확인하는 검증이 없다. require필요요
...
-->
만약 잔액이 1 ether라고 하고 이보다 더 큰 100 ether라는 값을 호출하면 잔액차감이후 언더플로우로 인해 매우 큰 값으로 설정되고, 컨트랜트계정 안의 100 ether를 빼갈 수 있다.
...
-->
입금없이 출금을 시도, withdraw를 호출하면 위에서 작성한 것처럼 require조건에 걸려 revert하게 된다.
Foundry에서는 기존적으로 revert시 FAIL처리된다. 따라서 vm.expectRevert를 추가해야한다.
...
- [O] 트랜잭션 필드(nonce, gasPrice, gasLimit 등)의 역할을 이해했다
- [O] 디지털 서명의 세 가지 보장(인증, 무결성, 부인 방지)을 설명할 수 있다
- [O] Private Key 보안의 중요성을 이해했다
- [O] Foundry 테스트 기본 패턴(vm.prank, vm.deal, assertEq)을 사용할 수 있다
- [O] require 조건의 필요성을 이해했다
