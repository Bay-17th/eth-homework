...
--> B, gasprice*가스 사용량= 총 가스 비용이다.
gasLimit 은 가스 사용의 상한선이다. 
...
--> B, nonce는 트랜잭션의 중복 방지 장치이기 때문에 가장 우선된다. gasPrice는 남의 트랜잭션보다 빨리 체결되기 위함이지 본인의 트랜잭션 순서를 바꾸기 위함이 아니다.
...
-->A, 인증은 서명을 오직 개인키를 가진 본인만 만들 수 있어 위조 불가능을 보장하고, 
무결성은 트랜잭션의 내용이 조금도 바뀌지 않았다는 것, 
부인 방지는 본인이 서명을 함으로써 만든 거래에 대해 자신의 의지를 보여준다.
...
-->재사용 공격은 누군가 서명된 트랜잭션을 복사해 여러번 보내는 것이다. nonce값을 도입함으로써 중복된 트랜잭션을 구분할 수 있게된다. 예를들어 nonce가 없다면 계속 보낼 수 있는 트랜잭션을 nonce:1로 한번 처리된 트랜잭션은 재사용이 불가하도록 만들 수 있다.
...
-->은행은 중앙 서버 1개에서만 거래를 취소하면 되지만, 블록체인은 모든 거래가 모든 노드에 기록되고, 블록들이 모두 이전 블록들을 가리키고 있기 때문에 거래 취소가 불가능하다. private key로 서명되고 confirm된 블록은 변경이 불가하다. 
...
이후: 어떤 부분이 달라졌나요?
-->이전엔 가스 가격을 사용자가 설정했지만, 이후에는 basefee(기본 요금)+ priorityFee(팁)으로 최소 요금이 생겼다. 여기서 basefee는 채굴자에게 가지 않고 소각된다.
...
-->vm.prank가 없으면 simplestoragetest가 실행되지만 user가 입금하는 상황 테스트를 위해서 sender를 강제로 user로 바뀌주는 코드이다.
storage_.deposit{value: 1 ether}() 는 deposit함수는 돈을 보내지 않는다. 돈을 입급하기 위해 중괄호를 사용한다.
...
-->위 상황에서 잔액이 1-100이 되어 엄청나게 큰 양수가 된다. (uint256은 음수가 될 수 없으므로)
...
contract Wallet {
    ...
    function withdraw(uint256 amount) public {
        require(balances[msg.sender] >= amount, "Insufficient balance"); //추가
        // 잔액 차감
        balances[msg.sender] -= amount;
...
--> 이 코드는 논리적으로 출금이 거부되어야 하는데, 에러가 난다. (테스트 실패)
...
contract SimpleStorageTest is Test {
    ...
    function test_WithdrawFails() public {
        // 입금 없이 바로 출금 시도
        vm.expectRevert();//다음 줄은 에러가 나야 성공이다라는 뜻
        storage_.withdraw(1 ether);
    }
}
```
