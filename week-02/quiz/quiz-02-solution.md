...
-->
B
gasPrice는 가스 한 단위당 지불할 의사가 있는 가격이며, gasLimit은 이 트랜잭션이 소모할 수 있는 최대 가스 양이다.
...
-->
B
이더리움에서 논스는 순서를 보장한다. 따라서 아무리 가스비가 높아도 더 낮은 논스의 트랜잭션을 먼저 처리한다.
...
-->
B
인증은 이 메시지가 진짜 키 소유자로부터 왔음을 증명하고, 부인 방지는 서명자가 나중에 자신이 서명한게 아니라고 발뺌할 수 없게 한다.
...
-->
C
타원곡선 암호화와 해시 함수를 사용하기 때문에 순방향 계산은 쉽지만, 역방향은 수학적으로 불가능에 가깝다.
...
-->
논스는 재사용 공격을 방지하기 위해 필요하다. 만약 논스가 없다면 과거에 성공했던 트랜잭션을 복사하여 네트워크에 여러 번 다시 제출하는 공격이 가능하다.
...
-->
블록체인은 중앙 관리자가 없는 비가역성 특성을 가진다. 따라서 은행은 해킹 시 거래 취소나 계좌 동결이 가능하지만, 프라이빗 키가 유출되면 해당 주소의 모든 권한이 해커에게 넘어가며 전송된 자금을 물리적으로 되찾을 방법이 없다.
...
이후: 어떤 부분이 달라졌나요?
-->
기존의 최고가 경매 방식에서 알고리즘 기반 수수료 방식으로 변경되었다. 네트워크 상황에 따라 결정되어 소각되는 baseFee와, 채굴자에게 빨리 처리를 부탁하며 주는 팁인 priorityFee로 분리되어 수수료 예측이 훨씬 쉬워졌다.
...
-->
vm.prank(user) : 다음 호출을 user주소로 실행하게 강제한다.
{value: 1 ether} : 함수 호출 시 실제로 이더를 실어 보낼 때 사용하는 솔리디티 문법이다.
...
어떤 검증이 빠져 있나요?
-->
사용자의 잔액이 충분한지 확인하는 require문이 없다.
...
-->
코드에서 잔액보다 큰 금액을 요구하면, 가스비만 낭비되고 트랜잭션이 취소된다.
이전 버전의 경우, 언더플로우 발생 시 매우 큰 양수로 바뀌어버리는 버그가 발생할 수 있다.
...
-->
SimpleStorage 내부에 잔액을 체크하는 require문이 있다면 잔액 부족으로 실패할 것이고, 없다면 전송할 이더리움이 컨트랙트에 없어서 실패한다.
